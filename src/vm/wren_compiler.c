...
static bool finishBlock(Compiler* compiler)
{
  ...
  // Empty blocks (with just some newlines inside) do nothing.
  if (match(compiler, TOKEN_RIGHT_BRACE)) return false;
                                   ...
                                   int numParams, char leftBracket, char rightBracket)
{
  ...
  // This function may be called with too many arguments. When that happens,
...
static void parameterList(Compiler* compiler, Signature* signature)
{
  ...
  // Allow new line before an empty parameter list
  ignoreNewlines(compiler);
...
static void forStatement(Compiler* compiler)
{
  ...
  // Note that we expect three addLocal calls in the following code.
